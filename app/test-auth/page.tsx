            ...
            <button 
              onClick={async () => {
                try {
                  ...
                  console.log(`API响应: ${response.status} - ${response.ok ? '成功' : await response.text()}`)
                } else {
                  console.log('没有有效的token')
                }
              } catch (error) {
                console.log('API测试失败: ' + (error instanceof Error ? error.message : 'Unknown error'))
                }
              }}
