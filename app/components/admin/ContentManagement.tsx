...
export default function ContentManagement() {
  ...
  const [activeTab, setActiveTab] = useState<'documents' | 'categories' | 'conversations'>('documents')
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [documentsRes, categoriesRes, conversationsRes, usersRes] = await Promise.all([
  ...
  const handleBatchOperation = (operation: string, value?: any) => {
    ...
    switch (operation) {
      case 'delete':
        message = `确定要删除选中的 ${itemCount} 个${activeTab === 'documents' ? '文档' : activeTab === 'categories' ? '分类' : '对话'}吗？此操作不可撤销！`
        onConfirm = () => {
          if (activeTab === 'documents') { batchDeleteDocuments(selectedItems) }
          else if (activeTab === 'categories') { batchDeleteCategories(selectedItems) }
          // 对话删除可能需要特殊处理
        }
        break
      case 'publish':
